...
The previous box gives an example of use of `titled-frame' environment.
  ...
  \colorframed add the possibility to colorize separately the continuation
...
\emph{The current box is an example of \emph{`shaded'}
    environment; The
    \emph{shadecolor} must have been defined by user. See the
...
\item \emph{This is an example of \emph{`snugshade'}
    environment. It shares with \emph{`shaded'} the usage of \emph{shadecolor}.}
\end{snugshade}
...
\item To customize the `titled-frame' output, one may simply redefine the
  ...
  |\TitleBarFrame|.  Check the source file for some more information.  This is
  provided as an exception to the general rule that this package should take
  care only of fixing colour problems (addition of new environments
...
\begin{footnotesize}
  The alternative |\colorframedTitleBarFrame| was added only because the
  |\TitleBarFrame| has clear problems, notwisthstanding the fact already
  mentioned that anyhow `titled-frame' had been broken some years ago by an
  upstream \LaTeX\ change.\par
\end{footnotesize}
% \color{RawSienna}
...
\end{verbatim}
which expands by default to nothing at all (meaning the continuation label
inherits the TFFrameColor colour).  There is also
\begin{verbatim}
\colorframedbordercolorcommand
\end{verbatim}
which expands by default to |\normalcolor|.  It influences the colour
of the framing done by environment `framed',
the original having no such customizability.  Do not redefine it to do nothing, it always should set a colour, else the colour of some borders may be lost.

For example
\begin{verbatim}
\setlength{\FrameRule}{5pt}
\setlength{\FrameSep}{9pt}
\renewcommand\colorframedbordercolorcommand{\color{gray}}
\end{verbatim}
