...
% pas de \footnote dans les environnements de framed...
  \normalcolor \emph{(the current box is an example of \emph{`shaded*'}
    environment; next we use \emph{`snugshade'} to wrap some list items. Both
    use a \emph{shadecolor} which must have been defined by user)}
\par
\end{footnotesize}
\end{shaded*}
...
\begin{enumerate}[noitemsep,font=\normalcolor]
\begin{snugshade}
\item
